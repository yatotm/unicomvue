...
根目录 `pnpm test` 会连同前端的 154 个用例一并运行（合计 242 个），额外覆盖 Vite 开发代理、`localStorage` 读写、账号保留、余量与 QCI 归一化、签约速率的多来源取值、已订业务分组、到期分组的展示模型、登录流程、运营商验证、截图分享、滚动锁定、页头滚动表面与界面契约。测试不会发送真实短信。
