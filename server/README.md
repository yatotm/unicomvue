...
面板对外的卖点是把联通不显示的 QCI 与峰值速率算出来，而**这一层刻意不参与那个计算**。网关只转述事实：在用业务清单、是否取到清单、限速标志、可用最高速率，以及联通万一真给了的 QCI 数字；推断全部留给浏览器，这样推错了是一段读得到的纯函数，而不是一个藏在服务端的黑盒。这条分工连同白名单细节，见下文「速率与 QCI」。
...
| `POST /qci_proxy/` | 已订业务：网络质量业务、业务清单标志、限速状态、可用最高速率、在用业务清单（含业务名里解析出的下行峰值）。QCI 推断不在这一层 |
| `GET /healthz` | 健康检查 |
