...
test("body and accent text clear AA on every surface, at rest and under the hover layer", () => {
  for (const [theme, scope] of THEMES) {
    for (const ink of ["--ui-on-surface", "--ui-on-surface-variant", "--ui-primary-ink", "--ui-danger-ink"]) {
...
test("no text is painted on the shell chrome: the canvas and the top bar carry controls only", () => {
  ...
  for (const [name, constant] of [
    ["主按钮", /const PRIMARY_BUTTON_CLASS = `[^`]*\bbg-primary\b[^`]*`/],
    ["次按钮", /const QUIET_BUTTON_CLASS = `[^`]*\bbg-surface-raised\b[^`]*`/],
  ]) {
    assert.match(bar, constant, `${name} must bring its own surface onto the shell`);
  }
...
// Warm tinted surfaces are what stop the panel reading as a sheet of white paper, so the
// levels have to stay separable and the tint has to stay warm.
test("the surface ladder keeps a visible warm step between canvas, card and overlay", () => {
  for (const [theme, scope] of THEMES) {
    const canvas = luminance(scope, "--ui-background");
    const card = luminance(scope, "--ui-surface");
    const overlay = luminance(scope, "--ui-surface-raised");

    assert.ok(card > canvas, `${theme} surface must sit above background`);
    assert.ok(overlay > card, `${theme} surface-raised must sit above surface`);
    // 「内部的划分太朴素了」的答案是嵌套圆角区域，那一步必须真的看得见：卡片托盘和它里面的
    // 纸色区域之间的对比不足 1.15 时，圆角就白画了。（更严的那条在下面的相邻表面规则里。）
    const step = (Math.max(overlay, card) + 0.05) / (Math.min(overlay, card) + 0.05);
    assert.ok(step >= 1.15, `${theme} nested region only steps ${step.toFixed(3)}× off the card tray`);
    assert.notEqual(resolveToken(scope, "--ui-surface"), "#ffffff", `${theme} surface must be tinted`);

    for (const token of ["--ui-background", "--ui-surface", "--ui-surface-raised", "--ui-surface-sunken"]) {
...
const SURFACE_PAIRS = [
  ...
  ["画布(最糟点) / 外层卡片", (s) => canvasRange(s).lightest, "--ui-surface", 1.2, 6],
  ["外层卡片 / 嵌套区域", "--ui-surface", "--ui-surface-raised", 1.2, 6],
  ["嵌套区域 / chip・轨道・输入框", "--ui-surface-raised", "--ui-surface-sunken", 1.2, 6],
  ["画布(最糟点) / 侧栏", (s) => canvasRange(s).lightest, (s) => railRange(s).lightest, 1.1, 4],
  ["画布(最暗点) / 侧栏", (s) => canvasRange(s).darkest, (s) => railRange(s).darkest, 1.1, 4],
  ["画布(最糟点) / 顶栏", (s) => canvasRange(s).lightest, (s) => shellRange(s, "--ui-topbar").lightest, 1.1, 4],
  ["画布(最暗点) / 顶栏", (s) => canvasRange(s).darkest, (s) => shellRange(s, "--ui-topbar").darkest, 1.1, 4],
  ["侧栏(最糟点) / 当前导航项", (s) => railRange(s).lightest, "--ui-surface-raised", 1.2, 6],
  ["顶栏(最糟点) / 栏上的按钮", (s) => shellRange(s, "--ui-topbar").lightest, "--ui-surface-raised", 1.2, 6],
  ["画布(最糟点) / 底部标签栏", (s) => canvasRange(s).lightest, "--ui-surface-raised", 1.2, 6],
  ["嵌套区域 / hover 覆盖层", "--ui-surface-raised", (s) => hovered(s, "--ui-surface-raised"), 1.05, 2.5],
  ...
  ["外层卡片 / 状态徽章", "--ui-surface", "--ui-primary-container", 1, 6],
  ["外层卡片 / 限速徽章", "--ui-surface", "--ui-danger-container", 1, 6],
  // 推断徽章画在**区域**里（到期 lane 的抬头），不在托盘上。旧契约只量了托盘上的那两枚。
  ["嵌套区域 / 推断徽章", "--ui-surface-raised", "--ui-warning-container", 1, 6],
  // 骨架屏是加载态里唯一的图形。它旧值和 chip 共用一个 token，于是在托盘上只剩 1.109×
  // （浅）、在区域里只剩 1.151×（深）：没人报，但「看不见的那一块」正是同一个病。
  ["嵌套区域 / 骨架块", "--ui-surface-raised", "--ui-skeleton", 1.2, 6],
  ["外层卡片 / 骨架块", "--ui-surface", "--ui-skeleton", 1.2, 6],
  ["对话框 / 被遮住的页面", "--ui-surface-raised", (s) => scrimmed(s), 1.5, 12],
  ["页面 / 它自己的遮罩", "--ui-surface", (s) => scrimmed(s), 1.2, 6],
];
...
const SHELL_PEER_MIN_DELTA = 2.5;
const SHELL_PEER_MAX_DELTA = 5.5;
const SHELL_PEERS = [
  ["侧栏 / 顶栏（画布最糟点一端）", "--ui-rail", "--ui-topbar", "lightest"],
  ["侧栏 / 顶栏（画布最暗点一端）", "--ui-rail", "--ui-topbar", "darkest"],
];
...
function scrimmed(scope) {
  return composite(parseColor(resolveToken(scope, "--ui-scrim")), { rgb: colorOf(scope, "--ui-surface") });
}
...
test("the rail and the top bar are two thicknesses of one veil, and the bar is the thin one", () => {
  for (const [theme, scope] of THEMES) {
    const [rail, topbar] = SHELL_VEILS.map(([, token]) => parseColor(resolveToken(scope, token)));
    assert.deepEqual(topbar.rgb, rail.rgb, `${theme} the two shell veils must be the same tint`);
    assert.ok(topbar.alpha < rail.alpha, `${theme} the top bar must be the thinner veil`);
    for (const [name, veil] of [["rail", rail], ["topbar", topbar]]) {
      assert.ok(veil.alpha > 0 && veil.alpha < 1, `${theme} the ${name} veil must stay translucent`);
    }
    // 「透」不是一个形容词：每一层 veil 自己的颜色都必须沿着长度在变，否则它和一块死色
    // 没有区别——正是 §16.1 里那个被否掉的 backdrop-filter（0.07% 像素、3/255）的形状。
    for (const [name, token] of SHELL_VEILS) {
      const range = shellRange(scope, token);
      const spread = Math.max(...range.lightest.map((channel, index) => Math.abs(channel - range.darkest[index])));
      assert.ok(spread >= 4, `${theme} the ${name} veil's own colour only varies ${spread.toFixed(1)}/255`);
    }
  }
});

// wash 的振幅不是自由的：它可以给画布方向和层次，但它**不许把画布推向卡片**。
// 上一轮暖 wash 是 rgb(255 252 243 / .7)，合成出来比卡片还亮——这就是卡片上边缘消失的
// 直接原因。上面那条规则已经用最糟点钉住了它，这里再单独说明一遍它属于哪一类问题。
test("the canvas washes may move the canvas, but never toward the card", () => {
  for (const [theme, scope] of THEMES) {
    const canvas = canvasRange(scope);
    const card = relativeLuminance(colorOf(scope, "--ui-surface"));
    assert.ok(
      relativeLuminance(canvas.lightest) < card,
      `${theme} the washed canvas overshoots the card it is supposed to sit under`,
    );
    // 而且它必须**真的**在变，否则侧栏的半透明就又变成了空操作（§16.1）。
    const spread = Math.max(...canvas.lightest.map((channel, index) => Math.abs(channel - canvas.darkest[index])));
    assert.ok(spread >= 8, `${theme} the canvas only varies ${spread.toFixed(1)}/255 — the rail has nothing to sample`);

    const rail = railRange(scope);
    const railSpread = Math.max(...rail.lightest.map((channel, index) => Math.abs(channel - rail.darkest[index])));
    assert.ok(railSpread >= 4, `${theme} the rail's own colour only varies ${railSpread.toFixed(1)}/255`);

    // 而且 wash 也不许被「修画布太深」的方向压扁：暖的那一道是侧栏沿着高度唯一能采到的
    // 东西，它的振幅一旦掉到 §16.1 里那个被否掉的 3/255 附近，半透明就又变成了空操作。
    // 所以画布的底色不是自由的：它 = 天花板 − 暖 wash 的振幅。
    const base = colorOf(scope, "--ui-background");
    const warm = composite(parseColor(resolveToken(scope, "--ui-canvas-warm")), { rgb: base });
    const warmSpread = Math.max(...warm.map((channel, index) => Math.abs(channel - base[index])));
    assert.ok(warmSpread >= 8, `${theme} the warm wash only lifts the canvas ${warmSpread.toFixed(1)}/255`);
  }
});
...
test("weight is a closed set of two, and 600 is spent on almost nothing", () => {
  const ALLOWED_BOLD = new Set([
    ...
    "src/components/AppTopBar.vue",    // 主按钮
    "src/components/ThemeSelector.vue", // 选中的分段
    "src/views/SettingsView.vue",      // 主按钮
  ]);
  ...
  for (const path of ALLOWED_BOLD) {
    ...
    for (const hit of source.matchAll(/font-semibold/g)) {
      ...
      assert.match(
        window,
        /route\.name === item\.name|themeMode === option\.value|bg-primary(?![\w-])|联通套餐查询/,
...
// 「理论上他们内部也可以是圆角卡片」：外层一张 8px 的托盘，里面每一块区域是自己的 6px 圆角卡，
// 铺在比托盘更亮一档的纸色上。区域没有阴影——整页仍然只有最外那一张浮起来。
test("every region inside the page card is a nested rounded card", () => {
  const panel = readSource("src/components/PagePanel.vue");
  const panelTag = panel.slice(panel.indexOf("<section"), panel.indexOf(">", panel.indexOf("<section")));
  assert.match(panelTag, /\brounded-control\b/, "a nested region is 6px — one step inside the 8px card");
  assert.match(panelTag, /\bbg-surface-raised\b/, "and it sits on the paper tone, not on the tray tone");
  assert.match(panelTag, /\boverflow-hidden\b/, "or its scrolling body squares off its own corners");
  assert.doesNotMatch(panelTag, /\bshadow-e/, "only the outer card is elevated");

  // 每一条路由里，卡片的直接内容要么是那一行标题，要么是一块 rounded-control 的纸色区域。
  const REGION = /rounded-control[^"]*bg-surface-raised|bg-surface-raised[^"]*rounded-control/;
  for (const path of [
    ...
    assert.match(readSource(path), REGION, `${path} must render its regions as nested rounded cards`);
  }

  // 托盘自己带内边距，这正是左下 / 右下曾经读起来是直角的根因：一块不透明、直角、和卡片
  // 同色的 .pane-scroll 正好压在角上。有了内边距，任何子元素都够不到那四个角。
  const card = readSource("src/components/AppCard.vue");
  assert.match(card, /rounded-card[^"]*bg-surface(?![\w-])/, "the tray is the 8px card");
  assert.match(card, /(?:^|\s)p-\d/, "the tray must pad its children away from its own corners");
  assert.match(card, /\bgap-\d/, "regions are separated by a gap, not by a rule");
});
...
test("the categorical set is fixed-order, per-entity and never colour-only", () => {
  ...
  // 每一条上色的条都印着自己的数值，并带一个可读的名字。
  const hero = readSource("src/components/DashboardHero.vue");
  assert.match(hero, /aria-label="`\$\{slot\.label\}，条形占比/, "a coloured meter must say what it is");
...
test("panels that share a row share a bottom edge", () => {
  ...
  const grid = dashboard.slice(dashboard.indexOf('class="grid min-w-0'), dashboard.indexOf("</AppCard>"));

  assert.match(grid, /@\[60rem\]:grid-cols-\[minmax\(0,1fr\)_21\.25rem\]/, "two columns from 60rem up");
  assert.match(grid, /\bgap-2\b[^"]*\bsm:gap-3\b/, "regions are separated by one gap, not by hairlines");
  assert.doesNotMatch(grid, /\bborder-t\b|\bborder-l\b/, "a nested rounded region draws its own edge; no rules between them");
  ...
  for (const cell of [
    ...
    assert.ok(grid.includes(cell), `the ${cell} cell must be placed explicitly, so the row stretches together`);
  }
  ...
  const panel = readSource("src/components/PagePanel.vue");
  assert.match(panel, /:class="\[bodyClass,/, "PagePanel puts the ceiling on .pane-scroll, not on the section");

  // 骨架屏必须用同一套落位，否则数字落地时会跳一下。
  ...
  for (const cell of [
    ...
    assert.ok(skeleton.includes(cell), `the skeleton must reproduce the ${cell} cell`);
  }
  ...
  assert.match(services, /\bgap-2\b[^"]*\bsm:gap-3\b/, "the two columns are divided by the same gap as everywhere else");
});
...
const SCROLL_OWNERS = [
  ...
  ["src/components/PagePanel.vue", 1, "pane", "区域内部的面板：滚到头必须把滚动交还给主人"],
];
const SCROLL_RULE = {
...
test("a shared screenshot lifts every height cap for the frame it captures", () => {
  assert.match(MAIN_CSS, /\.is-capturing \.page-panel,\s*\.is-capturing \.pane-scroll\s*\{\s*max-height:\s*none;/);
  const layout = readSource("src/views/AppLayout.vue");
...
test("the density fix is leading and rhythm, not a bigger type ramp", () => {
  ...
  // 内边距是第二根杠杆，而且现在是两层叠出来的：外层托盘 + 区域自己。文字到卡片外沿的
  // 实际距离必须不小于当初定死的 20 / 24，否则「更密了」就会从这里偷偷回来。
  const panel = readSource("src/components/PagePanel.vue");
  const card = readSource("src/components/AppCard.vue");
  const scale = (name, source, prefix) => {
    const hit = source.match(new RegExp(`(?:^|[\\s"])${name}:${prefix}-(\\d+(?:\\.\\d+)?)`))
      ?? source.match(new RegExp(`(?:^|[\\s"])${prefix}-(\\d+(?:\\.\\d+)?)`));
    assert.ok(hit, `${prefix} not found`);
    return Number(hit[1]) * 4;
  };
  const trayPad = { base: scale("", card, "p"), sm: scale("sm", card, "p") };
  const panelPad = { base: scale("", panel, "px"), sm: scale("sm", panel, "px") };
  assert.ok(trayPad.base + panelPad.base >= 20, `phone inset is ${trayPad.base + panelPad.base}px, under 20`);
  assert.ok(trayPad.sm + panelPad.sm >= 24, `pointer inset is ${trayPad.sm + panelPad.sm}px, under 24`);
  assert.match(panel, /\bpt-4\b/);
});
...
test("charts respect prefers-reduced-motion and never loop", () => {
  ...
  for (const file of ["ExpiryLanes", "DashboardHero", "ResourceTable"]) {
    const source = readSource(`src/components/${file}.vue`);
    assert.doesNotMatch(source, /animation:/, `${file} must not run a looping animation`);
    assert.match(source, /chart-animate/, `${file} must animate only when the data changes`);
  }
});
